...
{
   std::vector<int> val(set_size), weights(set_size); //max_weight);
	
   /* Generate vector contents randomly          */
   fill_vector_rand(val, 1, max_item_value);
   fill_vector_rand(weights, 1, max_item_weight);
	
   print_table_row<int>("input-values", val);
   print_table_row<int>("input-weights", weights);
	
   knapsack_01_dp(val, weights);
}
