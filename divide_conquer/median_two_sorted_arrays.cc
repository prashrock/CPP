...
{
   int mid = n/2;
   /* If n is odd, median is the middle element @idx n/2     */
   if(n % 2 == 1)   return v[mid];
   /* If n is even, median is avg of middle 2 elements       */
   else             return (v[mid-1] + v[mid])/2;
}

/* Divide and Conquer approach to find the median of two     *
 * sorted arrays of the same length. At each stage, split    *
 * both arrays into two and look at only one-half of the     *
 * array. The trick is to include n/2+1 elements because     *
 * median can straddle the n/2 boundary                      *
 * Note: This idea can easily be extended to cover the case  *
 * where both array lengths are different                    *
 * Time Complexity=O(lg n), Space Complexity=O(1)            */
template<typename T>
static inline T median(const T v1[], const T v2[], size_t n)
{
   /* First handle Divide and Conquer base cases             */
   if(n == 0)      return 0; 
   /* This base case is needed when array lens are unequal.  *
    * Note: even+even = odd+odd = even length                */
   else if(n == 1) return (v1[0] + v2[0]) / 2;
   /* Actual base-case. Note, median calculation must        *
    * consider 2 elements in each array                      */
   else if(n == 2)
      return (std::max(v1[0], v2[0]) + std::min(v1[1], v2[1])) / 2;

   /* Recursive cases:  Middle element of (n/2)+1 array      *
    * depends on value of n (even/odd)                       */
   int mid;
   int half_size = n/2 + 1;
   ...
   if (m1 == m2)  return m1;
   /* If first array's middle element is bigger we can ignore*
    *the 2nd half of first array                             */
   else if(m1 > m2) return median(&v1[0], &v2[mid], half_size);
   else             return median(&v1[mid], &v2[0], half_size);
}

/* Test Driver - Uses random numbers to test median above    */
void median_tester(size_t len, int count)
{
   cout << "Doing " << count << " median tests with length : "
        << len      << " - ";
