...
int measure_pthread_lock(unsigned num_of_iters=number_of_iterations)
{
   ...
   for (unsigned i = 0; i < num_of_iters; i++) {
      ...
      res = pthread_spin_unlock(&lock); /* unlock spinlock   */
   }
   tsc_measure_end(&tsc);
...
int measure_barebones_spinlock(unsigned num_of_iters=number_of_iterations)
{
   ...
   tsc_measure_start(&tsc);
   for (unsigned i = 0; i < num_of_iters; i++) {
      res = barebones_spinlock(&val);   /* take the spinlock */
      /* Avoid loop vectorization + act as compile time      *
       * memory barrier: http://goo.gl/T0RpLD                */
      __asm volatile("" ::: /* pretend to clobber */ "memory");
      res = barebones_spinunlock(&val); /* unlock spinlock   */
   }
   tsc_measure_end(&tsc);
   rt_measure_end(&time, true);
...
{
   if (argc != 2) { /* 2 arguments: prog_name [-single|-perf]*/
      print_usage(argv);
      return 1;
   }
